	...
	{		
		...
		{	
			...
			if ($key != 'item_format')
			{
				...
				{
					if(($con_info['ttr_item_id'] == "Name" || $con_info['ttr_item_id'] == "Subject") && $con_info['is_hidden'] == 'True'){
		...
		$f_ext = str_replace('.', '', $attr['filetype']);
		$f_code .= ' filetypes:'.$attr['filetype'];
		}
		if(isset($attr['filesize'])){
		$val = $attr['filesize'] * 1024;
		$val =  round($val);
		$f_code .= ' limit:'.$val.'kb';
		}
		$f_code .=  ']' ; 
		...
		$properties[] = '<div class="col-sm-8"><label class="contact_file btn-file"> '.$f_code.' Browse</label><span id="upload-file" class="filename"> No File Selected </span></div></div>';
		$properties[] = "\n";
		}
		...
		$post_mail = array(
			'subject' => sprintf(
				_x( '%1$s "%2$s"', 'mail subject', 'TT-7theme--0066' ),
				get_bloginfo( 'name' ), '['.$mail_arr[3].']' ),
			'sender' => sprintf( '['.$mail_arr[1].'] <%s>', '['.$mail_arr[2].']' ),
			'body' =>
				sprintf( __( 'From: %s', 'TT-7theme--0066' ),
					'['.$mail_arr[1].'] <['.$mail_arr[2].']>' ) . "\n"
				. sprintf( __( 'Subject: %s', 'TT-7theme--0066' ),
					'['.$mail_arr[3].']' ) . "\n\n"
				. __( 'Message Body:', 'TT-7theme--0066' )
					. "\n" . '[your-message]' . "\n\n"
				. '-- ' . "\n"
				. sprintf( __( 'This e-mail was sent from a contact form on %1$s (%2$s)',
					'TT-7theme--0066' ), get_bloginfo( 'name' ), home_url() ),
...
function tt_img_src($match)  {
	$uploads =  wp_upload_dir();
            ...
            return $src_attr . $quote . $uploads['url'] . '/' . str_replace(" ","-",$png) . $quote; // Sanchi Replaced space in image name by hyphen according to wordpress media library
 }
...
function tt_link_src($match) {
    ...
    global $wp_filesystem;
        ...
        while ($tt_read_image = readdir($tt_content_images)) {
            if ($tt_read_image != '.' && $tt_read_image != '..') {
                  $result = wp_upload_bits($tt_read_image, null, $wp_filesystem->get_contents($tt_images_dir . $tt_read_image));
                  if (isset($result['error']) && $result['error']) {
        			continue;
    				}
                  array_push($ttr_images,$result['file']);                
            }
        }
        $this->templatetoaster_import_content_image($ttr_images);
         return $result;
    }
    ...
    private function add_widget($sidebar, $blocktype, $title, $content = null, $tt_blockID, $listofpages)
    {
        ...
        if (array_filter($wp_widget, 'is_int', ARRAY_FILTER_USE_KEY)){
            $new_widget_id = max(array_keys($wp_widget))+1;
        }
	...
	// function created to start importing content images.
  	private function templatetoaster_import_content_image($ttr_images)
    {
		$parent_post_id = 0;
	...
	foreach($ttr_images as $path) {
    	...
    	$image_id = wp_insert_attachment($attachment, $path, $parent_post_id);
