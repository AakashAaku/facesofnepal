...
if (!function_exists('TemplateToaster_setup')) :
	 ...
	 */
        ...
        load_theme_textdomain("TT-7theme--0066", get_template_directory() . '/languages');
       ...
       // $TemplateToaster_options = get_option('TemplateToaster_theme_options');       // Priya:- shift to inside the TemplateToaster_theme_option function.
		...
		include_once( ABSPATH . 'wp-admin/includes/plugin.php' ); 
 		...
 		{
 			add_action( 'tgmpa_register', 'TemplateToaster_require_plugins' ); 
 		}
        ...
        */
        ...
        register_nav_menus(array(
            'primary' => __('Menu', "TT-7theme--0066"),
        ));
        ...
        register_default_headers(array(
            'wheel' => array(
                ...
                'description' => __('Wheel', "TT-7theme--0066")
            ),
            'shore' => array(
                'url' => '%s/images/headers/shore.jpg',
                'thumbnail_url' => '%s/images/headers/shore-thumbnail.jpg',
                /* translators: header image description */
                'description' => __('Shore', "TT-7theme--0066")
            ),
            'trolley' => array(
                'url' => '%s/images/headers/trolley.jpg',
                'thumbnail_url' => '%s/images/headers/trolley-thumbnail.jpg',
                /* translators: header image description */
                'description' => __('Trolley', "TT-7theme--0066")
            ),
            'pine-cone' => array(
                'url' => '%s/images/headers/pine-cone.jpg',
                'thumbnail_url' => '%s/images/headers/pine-cone-thumbnail.jpg',
                /* translators: header image description */
                'description' => __('Pine Cone', "TT-7theme--0066")
            ),
            'chessboard' => array(
                'url' => '%s/images/headers/chessboard.jpg',
                'thumbnail_url' => '%s/images/headers/chessboard-thumbnail.jpg',
                /* translators: header image description */
                'description' => __('Chessboard', "TT-7theme--0066")
            ),
            'lanterns' => array(
                'url' => '%s/images/headers/lanterns.jpg',
                'thumbnail_url' => '%s/images/headers/lanterns-thumbnail.jpg',
                /* translators: header image description */
                'description' => __('Lanterns', "TT-7theme--0066")
            ),
            'willow' => array(
                'url' => '%s/images/headers/willow.jpg',
                'thumbnail_url' => '%s/images/headers/willow-thumbnail.jpg',
                /* translators: header image description */
                'description' => __('Willow', "TT-7theme--0066")
            ),
            'hanoi' => array(
                'url' => '%s/images/headers/hanoi.jpg',
                'thumbnail_url' => '%s/images/headers/hanoi-thumbnail.jpg',
                /* translators: header image description */
                'description' => __('Hanoi Plant', "TT-7theme--0066")
            )
        ));        
    }
...
global $post, $product;
	...
	$label = '<span class="label label-primary sale">' . __( 'Sale!', "TT-7theme--0066") .'</span>';
	return $label;    
	}; 
...
<form method="post">
  <div class="update-nag">
    <?php _e('Import website content? &nbsp&nbsp&nbsp&nbsp', "TT-7theme--0066"); ?>
...
if (($_POST['importt'] != "")) {
    ...
    $imported = TemplateToaster_import_start();
    }
    if( $imported ) {
		 echo '<div class="updated"> <p>';
		  _e( "Content Imported", "TT-7theme--0066") ;
         echo'</p></div>';
        $imported = false;       
        update_option( 'is_import', '1' ); 
	}
	else {
		 echo '<div class="error notice"> <p>';
		  _e( "Content Not Imported Sucessfully, Reactivate and import it again", "TT-7theme--0066") ;
         echo'</p></div>';
        $imported = false;       
        update_option( 'is_import', '0' ); 
	}
...
function TemplateToaster_continue_reading_link()
{
    if (TemplateToaster_theme_option('ttr_read_more_button')) {
        return '<br/><br/><a href="' . esc_url(get_permalink()) . '">' . '<span class="btn btn-default">' .__( TemplateToaster_theme_option('ttr_read_more'), '', "TT-7theme--0066") . '<span class="meta-nav">&rarr;</span></span>'. '</a>';
    } else {
        return '<br/><br/><a href="' . esc_url(get_permalink()) . '">' . __(TemplateToaster_theme_option('ttr_read_more'), '', "TT-7theme--0066") . '<span class="meta-nav">&rarr;</span></a>';
...
<nav id="
  ...
  <h3 class="assistive-text">
    <?php echo(__('Navigation', "TT-7theme--0066")); ?>
  </h3>
 <?php endif; ?>
            <?php
            if ( ($nav_id == 'nav-above' && TemplateToaster_theme_option('ttr_pagination_link_posts_above')) || ($nav_id == 'nav-below' && TemplateToaster_theme_option('ttr_pagination_link_posts_below')) ){
                ...
                $pge = paginate_links(array(
                    ...
                    'prev_next' => True,
                    'format' => '?paged=%#%',
                    'prev_text' => __('Previous', "TT-7theme--0066"),
                    'next_text' => __('Next', "TT-7theme--0066"),
    ...
    <?php next_posts_link(__('<span class="meta-nav">&larr;</span> Older posts', "TT-7theme--0066")); ?>
  </div>
  <div
      class="nav-next">
    <?php previous_posts_link(__('Newer posts <span class="meta-nav">&rarr;</span>', "TT-7theme--0066")); ?>
  </div>
            <?php } ?>
</nav>
<!-- #nav-above -->
...
<li class="post pingback">
  <p>
    <?php _e('Pingback:', "TT-7theme--0066"); ?>
    <?php comment_author_link(); ?>
    <?php edit_comment_link(__('Edit', "TT-7theme--0066"), '<span class="edit-link">', '</span>'); ?>
  </p>
    <?php
    break;
    default :
...
<div>
    ...
    <div class="comment-author vcard">
        ...
        /* translators: 1: comment author, 2: date and time */
                            printf(__('%1$s on %2$s <span class="says">said:</span>', "TT-7theme--0066"),
                                sprintf('<span class="fn">%s</span>', get_comment_author_link()),
                                sprintf('<a href="%1$s"><time datetime="%2$s">%3$s</time></a>',
                                    esc_url(get_comment_link($comment->comment_ID)),
                                    get_comment_time('c'),
                /* translators: 1: date, 2: time */
                                    sprintf(__('%1$s at %2$s', "TT-7theme--0066"), get_comment_date(), get_comment_time())
            )
        );
        ?>
    ...
    <em class="comment-awaiting-moderation">
      <?php _e('Your comment is awaiting moderation.', "TT-7theme--0066"); ?>
    </em>
    <br/>
    <?php endif; ?>
    <!--
    ...
    <div class="reply">
      <?php comment_reply_link(array_merge($args, array('reply_text' => __('Reply <span>&darr;</span>', "TT-7theme--0066"), 'depth' => $depth, 'max_depth' => $args['max_depth']))); ?>
    </div>
    <!-- .reply -->
  </div>
  <!-- #comment-## -->
...
if (!function_exists('TemplateToaster_entry_meta')) :
    ...
    function TemplateToaster_entry_meta()
    {
        if (is_sticky() && is_home() && !is_paged())
            echo '<span class="featured-post">' . __('Sticky', "TT-7theme--0066") . '</span>';
        ...
        if (!has_post_format(array('chat', 'status'))):
            $categories_list = get_the_category_list(__(', ', "TT-7theme--0066"));
            if ($categories_list) {
        ...
        // Translators: used between list items, there is a space after the comma.
        $tag_list = get_the_tag_list('', __(', ', "TT-7theme--0066"));
        if ($tag_list) {
        ...
        if (!has_post_format(array('chat', 'status', 'aside', 'quote'))):
            if ('post' == get_post_type()) {
                printf('<span class="author vcard"><a class="url fn n" href="%1$s" title="%2$s" rel="author"> %3$s | </a></span>',
                    ...
                    esc_attr(sprintf(__('View all posts by %s', "TT-7theme--0066"), get_the_author())),
                    get_the_author()
                );
            }
        endif;
...
if (!function_exists('TemplateToaster_entry_date')) :
    ...
    {
        if (has_post_format(array('chat', 'status')))
            $format_prefix = _x('%1$s on %2$s ', '1: post format name. 2: date', "TT-7theme--0066");
        else
            $format_prefix = '%2$s ';
        ...
        if (TemplateToaster_theme_option('ttr_remove_date')):
            $date = sprintf('<span class="date"><a href="%1$s" title="%2$s" rel="bookmark"><time class="entry-date" datetime="%3$s">%4$s</time></a></span>',
                ...
                esc_attr(sprintf(__('Permalink to %s', "TT-7theme--0066"), the_title_attribute('echo=0'))),
    ...
    function TemplateToaster_posted_on($date, $author)
    {
        ...
        $post_url =  urldecode(esc_url( get_permalink() ) );
            ...
            if (is_sticky() && is_home() && !is_paged()) {
                ...
                echo '<span style="clear:both;">' . __('Sticky', "TT-7theme--0066") . '</span>';
            }
            if ( get_the_time() !== get_the_modified_time() ) 
			{
				if ($date && $author)
				{
					...
					{
						$time_string = '<a href="' . $post_url . '" rel="bookmark"><img alt="' . __('date', "TT-7theme--0066") . '" src="' . get_template_directory_uri() . '/images/datebutton.png"/><time class="updated" datetime="%3$s">%4$s</time><time class="entry-date post_updated published hidden" datetime="%1$s">%2$s</time></a><span class = "meta">  ' . __('by ', "TT-7theme--0066") . ' </span><span class="author vcard">  <img alt="' . __('author ', "TT-7theme--0066") . '" src="' . get_template_directory_uri() . '/images/authorbutton.png"/>  <a href="%5$s" title="%6$s" class="url fn n" rel="author">%7$s</a></span>';
					   $post_status= __('Updated on ', "TT-7theme--0066");
					}
					else if ($var_author)
					{
					  $time_string = '<span class = "meta">  ' . __('by ', "TT-7theme--0066") . ' </span><span class="author vcard">  <img alt="' . __('author ', "TT-7theme--0066") . '" src="' . get_template_directory_uri() . '/images/authorbutton.png"/>  <a href="%5$s" title="%6$s" class="url fn n" rel="author">%7$s</a></span>';
					}
					else if ($var_date)
					{
					   $time_string = '<img alt="' . __('date', "TT-7theme--0066") . '" src="' . get_template_directory_uri() . '/images/datebutton.png"/><time class="updated" datetime="%3$s">%4$s</time><time class="entry-date post_updated published hidden" datetime="%1$s">%2$s</time>';
					  $post_status= __('Updated on ', "TT-7theme--0066");
					}
				}
				else if ($date && !$author) 
				{
					if ($var_date && $var_author)
					{
						$time_string = '<a href="' . $post_url . '" rel="bookmark"><img alt="' . __('date', "TT-7theme--0066") . '" src="' . get_template_directory_uri() . '/images/datebutton.png"/><time class="updated" datetime="%3$s">%4$s</time><time class="entry-date post_updated published hidden" datetime="%1$s">%2$s</time></a><span class = "meta">  ' . __('by ', "TT-7theme--0066") . ' </span><span class="author vcard"> <a href="%5$s" title="%6$s" class="url fn n" rel="author">%7$s</a></span>';
					   $post_status= __('Updated on ', "TT-7theme--0066");
					}
					else if ($var_author)
					{
					  $time_string = '<span class = "meta">  ' . __('by ', "TT-7theme--0066") . ' </span><span class="author vcard"><a href="%5$s" title="%6$s" class="url fn n" rel="author">%7$s</a></span>';
					}
					else if ($var_date)
					{
					   $time_string = '<img alt="' . __('date', "TT-7theme--0066") . '" src="' . get_template_directory_uri() . '/images/datebutton.png"/><time class="updated" datetime="%3$s">%4$s</time><time class="entry-date post_updated published hidden" datetime="%1$s">%2$s</time>';
				...
				else
				{
					if ($var_date && $var_author)
					{
						$time_string = '<a href="' . $post_url . '" rel="bookmark"><time class="updated" datetime="%3$s">%4$s</time><time class="entry-date post_updated published hidden" datetime="%1$s">%2$s</time></a><span class = "meta">  ' . __('by ', "TT-7theme--0066") . ' </span><span class="author vcard"><a href="%5$s" title="%6$s" class="url fn n" rel="author">%7$s</a></span>';
					   $post_status= __('Updated on ', "TT-7theme--0066");
					}
					else if ($var_author)
					{
			...
			else
			{
				if ($date && $author)
				{
					if ($var_date && $var_author)
					{
						$time_string='<a href="' . $post_url . '" rel="bookmark"><img alt="' . __('date', "TT-7theme--0066") . '" src="' . get_template_directory_uri() . '/images/datebutton.png"/><time class="entry-date published" datetime="%1$s">%2$s</time><time class="post_published updated hidden" datetime="%3$s">%4$s</time></a><span class = "meta">  ' . __('by ', "TT-7theme--0066") . ' </span> <span class="author vcard"><img alt="' . __('author ', "TT-7theme--0066") . '" src="' . get_template_directory_uri() . '/images/authorbutton.png"/>   <a href="%5$s" title="%6$s" class="url fn n" rel="author">%7$s</a></span>';
					$post_status=  __('Posted on ', "TT-7theme--0066");
					}
					else if ($var_author)
					{
					  $time_string = '<span class = "meta">  ' . __('by ', "TT-7theme--0066") . ' </span><span class="author vcard">  <img alt="' . __('author ', "TT-7theme--0066") . '" src="' . get_template_directory_uri() . '/images/authorbutton.png"/>  <a href="%5$s" title="%6$s" class="url fn n" rel="author">%7$s</a></span>';
					...
					{
					   $time_string='<img alt="' . __('date', "TT-7theme--0066") . '" src="' . get_template_directory_uri() . '/images/datebutton.png"/><time class="entry-date published" datetime="%1$s">%2$s</time><time class="post_published updated hidden" datetime="%3$s">%4$s</time>';
					$post_status=  __('Posted on ', "TT-7theme--0066");
					}
				}
				else if ($date && !$author)
				...
				elseif (!$date && $author)
				{
					if ($var_date && $var_author)
					{
						$time_string='<a href="' . $post_url . '" rel="bookmark"><time class="entry-date published" datetime="%1$s">%2$s</time><time class="post_published updated hidden" datetime="%3$s">%4$s</time></a><span class = "meta">  ' . __('by ', "TT-7theme--0066") . ' </span> <span class="author vcard"><img alt="' . __('author ', "TT-7theme--0066") . '" src="' . get_template_directory_uri() . '/images/authorbutton.png"/>   <a href="%5$s" title="%6$s" class="url fn n" rel="author">%7$s</a></span>';
					$post_status=  __('Posted on ', "TT-7theme--0066");
					}
					else if ($var_author)
					{
				...
				else
				{
					if ($var_date && $var_author)
					{
						$time_string='<a href="' . $post_url . '" rel="bookmark"><time class="entry-date published" datetime="%1$s">%2$s</time><time class="post_published updated hidden" datetime="%3$s">%4$s</time></a><span class = "meta">  ' . __('by ', "TT-7theme--0066") . ' </span> <span class="author vcard"><a href="%5$s" title="%6$s" class="url fn n" rel="author">%7$s</a></span>';
					$post_status=  __('Posted on ', "TT-7theme--0066");
					}
					else if ($var_author)
					{
			...
			$time_string = sprintf( $time_string,
					...
					esc_url(get_author_posts_url(get_the_author_meta('ID'))),
					sprintf(esc_attr__('View all posts by %s', "TT-7theme--0066"), esc_html(get_the_author())),
					esc_html(get_the_author())
					);
					
					
			// Wrap the time string in a link, and preface it with 'Posted on' or 'Updated On'.
			printf(__( '<span class="meta">%s</span>%s', 'TT-7theme--0066' ),$post_status,$time_string );
...
function TemplateToaster_generate_menu($TemplateToaster_cssprefix = "ttr_", $meenu, $TemplateToaster_magmenu, $TemplateToaster_menuh, $TemplateToaster_vmenuh, $TemplateToaster_ocmenu)
{
    ...
    $output = '';
    if (is_front_page()) {
        $output .= '<li class="' . $TemplateToaster_cssprefix . $meenu . '_items_parent dropdown active"><a href="' . get_home_url(null, '/') . '" class="' . $TemplateToaster_cssprefix . $meenu . '_items_parent_link_active"><span class="menuchildicon"></span>' . __('Home', "TT-7theme--0066");
	   ...
	   /* $output .= ('</a><hr class="horiz_separator" />'); */
        ...
        $output .= '<li class="' . $TemplateToaster_cssprefix . $meenu . '_items_parent dropdown"><a href="' . get_home_url(null, '/') . '" class="' . $TemplateToaster_cssprefix . $meenu . '_items_parent_link"><span class="menuchildicon"></span>' . __('Home', "TT-7theme--0066");
...
function TemplateToaster_theme_comment_form($args = array(), $post_id = null)
{
    ...
    $fields = array(
        'author' => '<p class="comment-form-author">' . '<label for="author">' . __('Name', "TT-7theme--0066") . '</label> ' . ($req ? '<span class="required">*</span>' : '') . '<br/>' .
            '<input id="author" name="author" type="text" value="' . esc_attr($commenter['comment_author']) . '" size="30"' . $aria_req . ' /></p>',
        'email' => '<p class="comment-form-email"><label for="email">' . __('Email', "TT-7theme--0066") . '</label> ' . ($req ? '<span class="required">*</span>' : '') . '<br/>' .
            '<input id="email" name="email" type="text" value="' . esc_attr($commenter['comment_author_email']) . '" size="30"' . $aria_req . ' /></p>',
        'url' => '<p class="comment-form-url"><label for="url">' . __('Website', "TT-7theme--0066") . '</label>' . '<br/>' .
            '<input id="url" name="url" type="text" value="' . esc_attr($commenter['comment_author_url']) . '" size="30" /></p>',
    );

    $required_text = sprintf(' ' . __('Required fields are marked %s', "TT-7theme--0066"), '<span class="required">*</span>');
    $defaults = array(
        'fields' => apply_filters('comment_form_default_fields', $fields),
        'comment_field' => '<p class="comment-form-comment"><label for="comment">' . _x('Comment', 'noun', "TT-7theme--0066") . '</label><textarea id="comment" name="comment" cols="45" rows="8" aria-required="true"></textarea></p>' . '<br/>',
        'must_log_in' => '<p class="must-log-in">' . sprintf(__('You must be <a href="%s">logged in</a> to post a comment.', "TT-7theme--0066"), wp_login_url(apply_filters('the_permalink', get_permalink($post_id)))) . '</p>',
        'logged_in_as' => '<p class="logged-in-as">' . sprintf(__('Logged in as <a href="%1$s">%2$s</a>. <a href="%3$s" title="Log out of this account">Log out?</a>', "TT-7theme--0066"), admin_url('profile.php'), $user_identity, wp_logout_url(apply_filters('the_permalink', get_permalink($post_id)))) . '</p>',
        'comment_notes_before' => '<p class="comment-notes">' . __('Your email address will not be published.', "TT-7theme--0066") . ($req ? $required_text : '') . '</p>',
        'comment_notes_after' => '<p class="form-allowed-tags">' . sprintf(__('You may use these <abbr title="HyperText Markup Language">HTML</abbr> tags and attributes: %s', "TT-7theme--0066"), ' <code>' . allowed_tags() . '</code>') . '</p>',
        'id_form' => 'commentform',
        'id_submit' => 'submit',
        'title_reply' => __('Leave a Reply', "TT-7theme--0066"),
        'title_reply_to' => __('Leave a Reply to %s', "TT-7theme--0066"),
        'cancel_reply_link' => __('Cancel reply', "TT-7theme--0066"),
        'label_submit' => __('Post Comment', "TT-7theme--0066"),
    );
...
function TemplateToaster_add_init()
{
    ...
    if ($screen->id == 'appearance_page_mytheme-options') {
        ...
        $tt_upload_data = array('title' => __('Choose or Upload an Image', "TT-7theme--0066"), 'text' => __('Use this image', "TT-7theme--0066"));
...
function TemplateToaster_replace_thickbox_text($translated_text, $text, $domain)
{
    if ('Insert into Post' == $text) {
        ...
        if ($referer != '') {
            return __('Select this image!', "TT-7theme--0066");
        }
    }
...
function TemplateToaster_customAdmin()
{
    ...
    $passed_data = array('on' => __('ON', "TT-7theme--0066"), 'off' => __('OFF', "TT-7theme--0066"), 'pageClass' => $pageClass);
        }
        else
	  $passed_data = array('on' => __('ON', "TT-7theme--0066"), 'off' => __('OFF', "TT-7theme--0066"));
...
function TemplateToaster_wordpress_breadcrumbs()
    ...
    $name = __('Home', "TT-7theme--0066"); //text for the 'Home' link
    ...
    if (!is_home() && !is_front_page() || is_paged()) {
        ...
        if (is_category()) {
            ...
            echo $currentBefore . __('Archive by category &#39;', "TT-7theme--0066");
            single_cat_title();
            echo '&#39;' . $currentAfter;
            ...
            echo $currentBefore . __('Search results for &#39;', "TT-7theme--0066") . get_search_query() . '&#39;' . $currentAfter;

        } elseif (is_tag()) {
            echo $currentBefore . __('Posts tagged &#39;', "TT-7theme--0066");
            single_tag_title();
            echo '&#39;' . $currentAfter;
            ...
            echo $currentBefore . __('Articles posted by', "TT-7theme--0066") . $userdata->display_name . $currentAfter;

        } elseif (is_404()) {
            echo $currentBefore . __('Error 404', "TT-7theme--0066") . $currentAfter;
		}

        if (get_query_var('paged')) {
            if (is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author()) echo ' (';
            echo __('Page', "TT-7theme--0066") . ' ' . get_query_var('paged');
            if (is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author()) echo ')';
        }
...
function TemplateToaster_in_widget_form($t, $return, $instance)
{
    ...
    $wp_page_types = array(
        'front' => __('Front', "TT-7theme--0066"),
        'home' => __('Blog', "TT-7theme--0066"),
        'archive' => __('Archives', "TT-7theme--0066"),
        'single' => __('Single Post', "TT-7theme--0066"),
        '404' => __('404', "TT-7theme--0066")
    );

    ?>
  <br/>
  <label>
    <?php echo(__('Hide widget on:', "TT-7theme--0066")); ?>
  </label>
	 ...
	 <?php foreach ($wp_page_types as $key => $label){
      ...
      <div class="menupageelement">
        ...
        <label class="widgetlabel"
          ...
          "><?php echo $label . ' ' . __('Page', "TT-7theme--0066") ?>
        </label>
        </div>
    <?php } ?>
  ...
  <label for="
    <?php echo $t->get_field_id('style'); ?>"><?php echo(__('Block Style:', "TT-7theme--0066")); ?>
  </label>
  <select id="
    <?php echo $t->get_field_id('style'); ?>" name="<?php echo $t->get_field_name('style'); ?>">
    <option
      <?php selected($instance['style'], 'default'); ?>value="default"><?php echo(__('Default', "TT-7theme--0066")); ?>
    </option>
    <option
      <?php selected($instance['style'], 'none'); ?>
      value="none"><?php echo(__('None', "TT-7theme--0066")); ?>
    </option>
    <option
      <?php selected($instance['style'], 'block'); ?>value="block"><?php echo(__('Block', "TT-7theme--0066")); ?>
    </option>
        </select>
    <?php
    $retrun = null;
...
function TemplateToaster_post_options_array()
{
    $postoptions = array(
        array("type" => "open"),
        array("name" => __("Display Post Title", "TT-7theme--0066"),
            "desc" => "Check this box if you would like to DISABLE the Post Title",
            "id" => "ttr_post_title_checkbox",
            "type" => "checkbox",
            "std" => "true"),
        array("name" => __("Enable Post link", "TT-7theme--0066"),
...
function TemplateToaster_page_options_array()
{
    $pageoptions = array(
        array("type" => "open"),
        array("name" => __("Display Page Title", "TT-7theme--0066"),
            "desc" => "Check this box if you would like to DISABLE the Page Title",
            "id" => "ttr_page_title_checkbox",
            "type" => "checkbox",
            "std" => "true"),
        array("name" => __("Display Footer", "TT-7theme--0066"),
            "desc" => "Check this box if you would like to DISABLE the Page Footer",
            "id" => "ttr_page_foot_checkbox",
            "type" => "checkbox",
            "std" => "true"),
        array("name" => __("Header Background Style", "TT-7theme--0066"),
            "desc" => "Select Box for Header Image size",
            "id" => "ttr_header_size_select",
            "type" => "select",
            "std" => "none",
            "options" => array(__("None", "TT-7theme--0066"), __("Fill", "TT-7theme--0066"), __("Horizontal Fill", "TT-7theme--0066"), __("Vertical Fill", "TT-7theme--0066"))),
        array("name" => __("Disable header Image Repeat", "TT-7theme--0066"),
            "desc" => "Check this box if you dont want to repeat image",
            "id" => "ttr_background_repeat_enable_checkbox",
            "type" => "checkbox",
            "std" => "true"),
        array("name" => __("Header  Background Image", "TT-7theme--0066"),
            "desc" => "Choose Header Image",
            "id" => "ttr_change_header_image_text",
            "type" => "media",
            "std" => ""),
        array("name" => __("Body Background Style", "TT-7theme--0066"),
            "desc" => "Select Box for background Image size",
            "id" => "ttr_background_size_select",
            "type" => "select",
            "std" => "none",
            "options" => array(__("None", "TT-7theme--0066"), __("Fill", "TT-7theme--0066"), __("Horizontal Fill", "TT-7theme--0066"), __("Vertical Fill", "TT-7theme--0066"))),
        array("name" => __("Disable background Image Repeat", "TT-7theme--0066"),
            "desc" => "Check this box if you dont want to repeat image",
            "id" => "ttr_header_repeat_enable_checkbox",
            "type" => "checkbox",
            "std" => "true"),
        array("name" => __("Body Background Image", "TT-7theme--0066"),
...
function TemplateToaster_add_custom_box() {
    ...
    foreach($screens as $screen)
    {
        add_meta_box(
            'post_page_options',
            __( 'Theme Options',"TT-7theme--0066" ),
...
// to check whether gutenberg editor exists and save accordingly

// harsh added version chck in order to set the value of ttr_post_title_checkbox to true in vrs 5.0 or above (to show page title post titile)
         if(version_compare ( get_bloginfo('version') , '5.0', 'lt' ))
         {
...
function TemplateToaster_theme_option($option)
{
	...
	else
	{
		...
		return sanitize_text_field($TemplateToaster_options[$option]);
		}
	}
}
...
function TemplateToaster_wp_title($title, $sep)
{
    ...
    if (($paged >= 2 || $page >= 2) && !is_404()) {
        $title .= " $sep " . sprintf(__('Page %s', "TT-7theme--0066"), max($paged, $page));
    }
...
<span class="url"><a class="<?php echo $TemplateToaster_cssprefix; ?>author_name" href="<?php echo esc_url( get_comment_link( $comment->comment_ID ) ) ?>"></a><?php
    echo get_comment_author_link( $comment->comment_ID ); ?>
                ...
                <time datetime="<?php echo get_comment_date("c")?>" class="comment-date">
                    <a class="<?php echo $TemplateToaster_cssprefix; ?>comment_date" href="<?php echo esc_url( get_comment_link( $comment->comment_ID ) ) ?>"><?php printf(__('%1$s at %2$s', "TT-7theme--0066"), get_comment_date(),  get_comment_time()) ?></a> </time>
                <hr style="color:#fff;"/>
                <?php comment_text() ?>
                <div style="clear:both"></div>
                <hr style="color:#fff;">
                ...
                <div class="<?php echo $TemplateToaster_cssprefix; ?>comment_reply_edit">
                    ...
                    <span class="<?php echo $TemplateToaster_cssprefix; ?>reply_edit"><?php edit_comment_link(__('(Edit)', "TT-7theme--0066")) ?></span>
                </div>
                <?php if ($comment->comment_approved == '0') : ?>
                    <br /><em><?php _e('Your comment is awaiting approval.', "TT-7theme--0066") ?></em>
                <?php endif; ?>
            </div>
...
function TemplateToaster_require_plugins()
{
$plugins = array(
array(
'name' => 'Contact Form 7',
...
function theme_customize_register( $wp_customize ) {
	...
	$wp_customize->add_control( 'colorscheme', array(
		'type'    => 'radio',
		'label'    => __( 'Color Scheme', 'TT-7theme--0066' ),
		'choices'  => array(
			'light'  => __( 'Light', 'TT-7theme--0066' ),
			'custom' => __( 'Custom', 'TT-7theme--0066' ),
		),
		'section'  => 'colors',
		'priority' => 5,
	) );
...
function theme_enqueue_styles() {
  wp_enqueue_style( 'customizer-styles', get_stylesheet_uri() ); // This is where you enqueue your theme's main stylesheet
  $custom_css = theme_get_customizer_css();
  wp_add_inline_style( 'customizer-styles', $custom_css );
}
...
function templatetoaster_import_xml()
{
	...
	if ($_POST['data'] != NULL)
	{
		...
		$load_theme_options=simplexml_load_string($LoadXML1) or die(__('Error: Cannot create object','TT-7theme--0066'));
		...
		if ($load_theme_options->option)
		{
			...
			if($result)
			{
				die(__('Theme options imported successfully','TT-7theme--0066'));				
			}
			else
			{
				die(__('Problem in importing theme options.','TT-7theme--0066'));	
			}
		}
		else
		{
			die(__('Incorrect XML file.','TT-7theme--0066'));
		}	
	}
	else
	{
		die(__('Please, upload an XML file only','TT-7theme--0066'));
	}
}
...
function tt_style_load() {
    ...
    // enqueue style for Gutenberg editor
		add_editor_style(get_template_directory_uri() . '/css/bootstrap.css ', get_template_directory() .'/style.css' );
...
// getting theme properties path  
   function ttr_upload_list(){
		...
		$tt_favicon_path = get_template_directory(). '/favicon.ICO';
